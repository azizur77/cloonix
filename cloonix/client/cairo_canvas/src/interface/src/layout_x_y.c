                         ...
                         double *tx, double *ty, int *thidden_on_graph) 
{
  ...
  memcpy(nl->tx, tx, (MAX_PERIPH_VM)*sizeof(double));
  memcpy(nl->ty, ty, (MAX_PERIPH_VM)*sizeof(double));
  memcpy(nl->thidden_on_graph,thidden_on_graph,(MAX_PERIPH_VM)*sizeof(int));
                         ...
                         double *tx, double *ty, int *thidden_on_graph)
{
  ...
  if (cur)
    ...
    *hidden_on_graph = cur->hidden_on_graph;
    memcpy(tx, cur->tx, (MAX_PERIPH_VM)*sizeof(double));
    memcpy(ty, cur->ty, (MAX_PERIPH_VM)*sizeof(double));
    memcpy(thidden_on_graph,cur->thidden_on_graph,(MAX_PERIPH_VM)*sizeof(int));
    unset_node_layout(name);
    }
  else
    {
    ...
    *color_choice = 0;
    *hidden_on_graph = 0;
    memset(tx, 0, (MAX_PERIPH_VM)*sizeof(double));
    memset(ty, 0, (MAX_PERIPH_VM)*sizeof(double));
    memset(thidden_on_graph, 0,(MAX_PERIPH_VM)*sizeof(int));
    for (i=0; i<MAX_PERIPH_VM; i++)
      {
      rest = i%4;
      if (rest == 0)
