...
/*****************************************************************************/
int add_machine(char *name, int vm_id)
{
  int  result = -1;
  t_machine *ma = look_for_machine_with_name(name);
  if (!ma)
    {
    ma = (t_machine *) malloc(sizeof(t_machine));
    memset(ma, 0, sizeof(t_machine));
    ma->vm_id = vm_id;
    strncpy(ma->name, name, MAX_NAME_LEN-1);
    if (head_machine)
...
/*****************************************************************************/
int del_machine(char *name, int vm_id)
{
  int result = -1;
  t_machine *ma = look_for_machine_with_name(name);
  if (ma)
    {
    if (ma->vm_id != vm_id)
      KERR("%s %d %d", name, vm_id, ma->vm_id);
    clownix_timeout_del(get_all_ctx(), 
  ...
  while(cur)
    ...
    if (machine != cur)
      if (!strcmp(alloc_ip, cur->ip))
        KOUT("%s %s %s %s", alloc_ip, cur->ip, machine->name, cur->name);
    cur = cur->next;
    }
