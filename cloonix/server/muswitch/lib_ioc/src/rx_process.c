...
{
  dchan->boundary = (char *)malloc(strlen(boundary) + 1);
...
/*****************************************************************************/
static int get_close_boundary_from_chunk(t_all_ctx *all_ctx,
                                         t_data_chunk *first,
                                         char *boundary, 
                                               ...
                                               int *i_bound)
{
  ...
  if (found == 4)
    ...
    xml_boundary = (char *)malloc(nb_char_in_close_bound + 2);
...
/*****************************************************************************/
static int chunks_state_rx_type_close_bound_found(t_all_ctx *all_ctx,
                                                  ...
                                                  char **found, int *found_len)
{
  ...
  if (!dchan->boundary)
    KOUT(" ");
  res = get_close_boundary_from_chunk(all_ctx, dchan->rx,
                                      dchan->boundary, &hit_a_problem);
  if (res == 1)
    {
    ...
    if (end_found)
      ...
      *found_len += 1;
      if (len_what_is_left)
        {
        what_is_left = (char *) malloc(len_what_is_left+1);
...
/*****************************************************************************/
static void string_rx_to_process(t_all_ctx *all_ctx, t_data_channel *dchan)
{
  int found_len;
  char *found;
  ...
  while(dchan->decoding_state == rx_type_open_bound_found)
    {
    if (chunks_state_rx_type_close_bound_found(all_ctx, dchan, 
                                               &found, &found_len))
      {
      dchan->rx_callback(all_ctx, dchan->llid, found_len, found);
      free(found);
      if (dchan->llid)
        chunks_state_rx_type_ascii_start(dchan);
      else
                       ...
                       int len, char *new_rx)
{
  ...
  if ((dchan->decoding_state == rx_type_ascii_start)||
    ...
    string_rx_to_process(all_ctx, dchan);
