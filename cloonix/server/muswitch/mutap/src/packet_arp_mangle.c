...
/*****************************************************************************/
static char *get_mac_string(char *mac)
{
...
/*****************************************************************************/
static inline int dst_mac_is_unicast(char *data)
{
  return (!(data[0] & 0x1));
}
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
static inline int src_mac_is_unicast(char *data)
{
  return (!(data[6] & 0x1));
}
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
static char *get_arp_sender_mac(char *data)
{
  static char mac[6];
  int i;
  for (i=0; i<6; i++)
...
/*****************************************************************************/
static void get_arp_sender_ip(char *data, int *ip)
{
  *ip = 0;
...
/*****************************************************************************/
static void get_arp_target_ip(char *data, int *ip)
{
  *ip = 0;
...
/*****************************************************************************/
static inline char *get_src_mac(char *data)
{
  return (&(data[6]));
}
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
static inline char *get_dst_mac(char *data)
{
  return (&(data[0]));
}
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
static void put_arp_sender_mac(char *mac, char *data)
{
  int i;
  for (i=0; i<6; i++)
...
/*****************************************************************************/
static void put_arp_target_mac(char *mac, char *data)
{
  int i;
  for (i=0; i<6; i++)
...
/*****************************************************************************/
static int get_proto(int len, char *data)
{
...
/*****************************************************************************/
static void associate_privates(int ip, char *mac)
{
...
/*****************************************************************************/
static char *get_association(int ip)
{
  t_association *cur = head_association;
  char *mac = NULL;
  while (cur)
...
/****************************************************************************/
static int tst_ip_input(t_all_ctx *all_ctx, 
                        int len, char *data, int *real_len)
{
  int result = -1;
...
/*****************************************************************************/
int packet_arp_mangle(t_all_ctx *all_ctx, int is_sendto, int len, char *data)
{
  int result = 0;
  int proto, real_len;
  int private_net_ip;
  char *private_net_mac;
  if (len < 40)
    { 
    ...
    case proto_arp_resp:
      if (is_sendto)
        ...
        if (src_mac_is_unicast(data))
          put_src_mac(get_glob_wif_hwaddr(), data);
        put_arp_sender_mac(get_glob_wif_hwaddr(), data);
        }
      else
        {
      ...
      break;
    case proto_ip:
      if (tst_ip_input(all_ctx, len-MAC_HEADER, data+MAC_HEADER, &real_len))
        KERR("BAD IP PACKET");
      else
        {
        private_net_ip = 0;
        if (!is_sendto)
          ...
          if (private_net_mac)
            {
            if ((dst_mac_is_unicast(data)) &&
                (!memcmp(get_glob_wif_hwaddr(), get_dst_mac(data), 6)))
              put_dst_mac(private_net_mac, data);
            }
          }
        else
...
/*****************************************************************************/
void init_packet_arp_mangle(t_all_ctx *all_ctx)
{
  (void) all_ctx;
