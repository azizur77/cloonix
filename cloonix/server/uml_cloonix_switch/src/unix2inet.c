...
/*****************************************************************************/
static void send_event_broken_unix2inet(int llid, int llid_con, char *name)
{
  char msg[MAX_PATH_LEN];
  sprintf(msg, "unix2inet_conpath_evt_break llid=%d name=%s", llid_con, name);
...
{
  int nb, nb_lan_nat, nb_lan_vm, common_lan;
  int lan_vm[MAX_ETH_VM*MAX_TRAF_ENDPOINT];
  int lan_nat[MAX_TRAF_ENDPOINT];
  t_vm *vm;
  memset(lan_vm, 0, MAX_ETH_VM*MAX_TRAF_ENDPOINT*sizeof(int));
  memset(lan_nat, 0, MAX_TRAF_ENDPOINT*sizeof(int));
...
void unix2inet_lan_event(int llid, int llid_con)
{
  ...
  if (!rec)
    ...
    {
    send_event_broken_unix2inet(llid, llid_con, rec->vmname);
...
void unix2inet_llid_cutoff(int llid)
{
  ...
  while(rec)
    {
    send_event_broken_unix2inet(llid, rec->llid_con, rec->vmname);
...
void unix2inet_vm_cutoff(char *vmname)
{
  ...
  while(rec)
    {
    send_event_broken_unix2inet(rec->llid, rec->llid_con, vmname);
