...
/*****************************************************************************/
static int time_samples[MAX_TIME_SAMPLES];
...
static void store_time_sample(int delta)
{
  if (delta > 500)
    ...
    glob_above15ms += 1;
  time_samples[time_samples_idx] = delta;
  time_samples_idx++;
...
static t_sys_info *create_sys_event(void)
{
  ...
  for (i=0; i<type_llid_max; i++)
    sys->fds_used[i] = cfg_get_trace_fd_qty(i);
  ...
  sys->clients      = cfg_get_trace_fd_qty(type_llid_trace_client_unix);
...
static void loss_of_fd_trap(void)
{
  ...
  declared_fds = cfg_get_trace_fd_qty(0);
...
{
  memset (time_samples, 0, MAX_TIME_SAMPLES * sizeof(int));
  time_samples_idx = 0;
  last_delta = 0;
